<template>
  <Teleport to="#app">
    <Window @out="store.isShowMiniPlayer = false" @ext="store.isShowMiniPlayer = false" :locksize="true" width="268px"
      height="90px" controlbtn>
      <PlayInfo />
      <div v-bind="$attrs" class="mini-control">
        <button class="show-body-lyric" @click="showbodylyric">
          <svg t="1668149792128" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            ...
            <path
              ...
              :fill="musicStore.lyricControl.isShowBodyLyric || musicStore.lyricControl.isShowLyric ? '#6893df' : '#515151'"
              p-id="5428"></path>
          </svg>
        </button>
        <Control class="fb" />
...
import PlayInfo from "@/components/PlayerInfo.vue";
import Control from "@/components/layout/inner/Control.vue";
const store = useStore()
const musicStore = useMusicStore()
const showbodylyric = () => {
  let isShowBodyLyric = musicStore.lyricControl.isShowBodyLyric
  let isShowLyric = musicStore.lyricControl.isShowLyric
  if (isShowLyric) {
    return
  }
  if (!isShowBodyLyric && !isShowLyric) {
    musicStore.setLyric()
  }
  musicStore.lyricControl.isShowBodyLyric = !isShowBodyLyric
}
</script>
